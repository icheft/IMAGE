...
def get_dividend_yield(stock_id, year=None):
    url = f"https://tw.stock.yahoo.com/d/s/dividend_{stock_id.split('.')[0]}.html"
    page = requests.get(url)
    soup = BeautifulSoup(page.content, "html.parser")
...
def app_backup(stock_id):
    ...
    star = stock_id = st.sidebar.text_input("輸入起始時間", "2011-3-12")
    en = stock_id = st.sidebar.text_input("輸入終止時間", "2021-3-12")
    stock_df = stock_obj.history(start=star, end=en, auto_adjust=False)  # period="max"

    ock_df = stock_obj.history(start=star, end=en, auto_adjust=False)  # period="max"
    ...
    init_balance = int(st.sidebar.text_input("原本有多少錢", "3000"))
...
def app():
    ...
    stock_id = st.sidebar.text_input("請輸入股票代號(例:0050.TW)", "0050.TW")
    app_backup(stock_id)


# stock_id = st.text_input("請輸入股票代碼(例:0050.TW)")
