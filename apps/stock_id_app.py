...
def app(stock_id="0050.TW"):
    ...
    st.write(
        f"""+ 如果當初投資 {init_balance} 元，經過 {year_difference} 年後，變成 {total_balance[-1]} 元
+ 年化報酬率為 {cgar * 100}%
...
def app_px(stock_id="0050.TW"):
    ...
    stock_yearly_returns.index = stock_yearly_returns.index.strftime("%Y")
    beautify_syr = (
        stock_yearly_returns.reset_index()
        .dropna()
        .rename(columns={"Adj Close": "Percentage Change"})
    )
    ...
    fig = px.bar(
        ...
        x="Date",
        y="Percentage Change",
        hover_data=["Percentage Change", "Date"],
